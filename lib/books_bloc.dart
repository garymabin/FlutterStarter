...
class BooksBloc {
  ...
  factory BooksBloc(BookRepository repository) {
    ...
    final state = new Observable<LoadingAction>
      (pullToRefresh.stream)
  ...
  static Stream<BookListState> Function(LoadingAction) _buildBooksStateStream(
    ...
    return (LoadingAction loadingAction) {
      return new Observable<BookDTO>.fromFuture(repository
          .getBooks(loadingAction.startingIndex))
          .map<BookListState>((BookDTO resp) {
        return new BookListState(books: resp, hasError: false, isLoading:
        false
        );
